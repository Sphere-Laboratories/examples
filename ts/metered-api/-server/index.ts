...
class UnitsUsedInterface {
    ...
    public static getUnitsUsedForPeriod = async(_subscription: Subscription): Promise<number> => 10_000
...
app.get(
    ...
    async (req: Request, res: Response) => {
        ...
        const unitsUsed: number = await UnitsUsedInterface.getUnitsUsedForPeriod(subscription)
        subscription = await sphere.subscription.updateUnits({
            id: subscription.id,
            units: unitsUsed
        })
        console.log(`Updated subscription: ${subscription.id}'s billable units to ${subscription.billableUnits}`)
        res.status(200).json({})
    }
)

server.listen(8080, () => console.log(`Saas::billing::units:updater started on port`))
